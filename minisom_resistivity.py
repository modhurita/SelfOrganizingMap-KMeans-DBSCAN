# -*- coding: utf-8 -*-
"""
Created on Fri Aug 20 10:46:02 2021
...
from numpy import (array, unravel_index, nditer, linalg, random, subtract, max,
                   ...
                   empty, append, size, shape, newaxis, sum)
...
#Iteration indices for incomplete data, each data vector is a stack
def _build_iteration_indexes_incomplete_data(num_iterations, good_data_indices, 
                                             verbose=False, random_generator=None):
    """Returns an iterable with the indexes of the samples
    to pick at each iteration of the training.
    ...
    data_len = size(good_data_indices)
    num_copies_data = int(num_iterations/data_len)
    num_indices_leftover = num_iterations % data_len

    iterations = empty(0)
    
    for i in range(num_copies_data):
        iterations = append(iterations, good_data_indices)

    iterations = append(iterations, good_data_indices[0:num_indices_leftover])
...
class MiniSom(object):
    def __init__(self, x, y, input_len, sigma=1.0, learning_rate=0.5,
        ...
        distance_functions = {'euclidean': self._euclidean_distance,
                              ...
                              'euclidean_masked': self._euclidean_distance_masked_data}
    ...
    def _euclidean_distance_masked_data(self, x, w):
        distance = sqrt(sum(((x-w)**2),axis=2))
        #print(x.shape)
        #print(w.shape)     
        #print((x-w).shape)
    ...
    #Update masked data
    def update_masked_data(self, x, win, t, max_iteration):
        """Updates the weights of the neurons.
        ...
        #broadcast g to be compatible with x
        g = g[:,:,newaxis]
        
        #update weights
        self._weights += g*(x-self._weights)
        #self._weights += einsum('ij, ijk->ijk', g, x-self._weights)
    ...
    #Train SOM when input data is incomplete
    def train_incomplete_data(self, data, num_iteration, good_data_indices, random_order=False, verbose=False):
        """Trains the SOM.
        ...
        if random_order:
            random_generator = self._random_generator
        iterations = _build_iteration_indexes_incomplete_data(num_iteration, good_data_indices,
                                              verbose, random_generator)
    ...
    #Train SOM when input data is masked
    def train_masked_data(self, data, num_iteration, random_order=False, verbose=False):
        """Trains the SOM.
        ...
        iterations = _build_iteration_indexes(len(data), num_iteration,
                                              verbose, random_generator)
        for t, iteration in enumerate(iterations):
            self.update_masked_data(data[iteration], self.winner(data[iteration]),
                        t, num_iteration)
        if verbose:
            print('\n quantization error:', self.quantization_error(data))
